import json, pyautogui, chromedriver_autoinstaller
import os
from tkinter import *
...
def build_page(template):
    ...
    for item in template[2:]:
        if item[0] == 'title':
            ...
            currlayout.append(canvas.create_image(475, y-50, image=item[1], anchor=N))
        elif item[0] == 'text':
...
def change_setts(*args):
    ...
    os.system("notepad.exe sets.txt")
...
chungus = PhotoImage(file='chungus.png')
chungus = chungus.subsample(5)
...
Main = ['main', None, ['title','Chungus is now here!'], ['img',chungus]]
currlayout = []
