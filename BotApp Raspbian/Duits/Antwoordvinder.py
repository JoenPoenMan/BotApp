...
def login(br, usr, pas):
    ...
    br.execute_script("document.getElementsByClassName('searchbox')[0].value='Dalton lyceum Barendrech'")
    ...
    br.execute_script("document.getElementById('ctl00_ContentPlaceHolder1_Username_input').value='{}'".format(usr))
    br.execute_script("document.getElementById('ctl00_ContentPlaceHolder1_Password_input').value='{}'".format(pas))
...
def waituntildone(i, br):
    while not 'block' in br.find_element_by_xpath('//div[@id="pages-container"]/div/div[{}]/div[contains(@class, "page-buttons")]/div[@id="action-buttonfollowing"]'.format(i)).value_of_css_property('display'):
        time.sleep(0.02)
